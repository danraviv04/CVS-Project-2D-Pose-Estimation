...
def parse_stem(path: Path) -> Tuple[str, int]:
    """
    Parse a label filename stem like <base>_<frame>, e.g. 4_2_24_B_2_000123.txt
    Return (base, frame) where frame is int.
    Raise ValueError if not matching.
    """
...
def enhance_like_infer(img_bgr: np.ndarray, clip=2.0) -> np.ndarray:
    """
    Apply CLAHE to the Y channel of the input BGR image,
    similar to what is done in YOLOSeg inference.
    clip: CLAHE clip limit (float > 0, typical 1.0-4.0)
...
"""
    ...
    if log_rows:
        ...
        print("No rows to write into curation_log.csv (after final filtering).")

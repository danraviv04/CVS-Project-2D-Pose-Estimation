...
def kind_from_name(name: str) -> str:
    """
    Returns "NH" or "T" based on the class name.
    Defaults to "T" if name is None or unrecognized.
    0: "NH" (Needle Holder)
    1: "T"  (Tweezers)
    """
    n = (name or "").lower()
...
def _pca_axes(P: np.ndarray):
    """
    PCA on 2D points to get major/minor axes.
    Returns: mean, major_axis, minor_axis, span_along_major, projections_along_major
    """
    mean = P.mean(0)
    A = P - mean
    C = np.cov(A.T)
...
def draw_kpts(img: np.ndarray, kps: Dict[str, List[float]], kind: str,
    ...
    """
    Overlays keypoints on the image.
    - img: HxWx3 BGR image (drawn in-place and also returned for convenience)
    - kps: dict of keypoint name → [x, y]
    - kind: "NH" or "T"
    - color: BGR tuple
    - r: circle radius
    - t: line thickness
    Returns the image (same as input img).
    """
...
def write_seg_txt(res, names_map: Dict[int, str], lab_dir: str, stem: str, save_conf=False):
    ...
    Uses Ultralytics' res.masks.xyn (list of arrays per instance).
    0-based cls IDs (int) are used; names_map is not needed here but
    could be used for debugging/verification if desired.
    0: "NH" (Needle Holder)
    1: "T"  (Tweezers)
    2: "other" (if present)
    3: "background" (if present)
    4: "artifact" (if present)
    5: "hand" (if present)
    6: "suture" (if present)    
    """
